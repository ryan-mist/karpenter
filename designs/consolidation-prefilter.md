...
**When it bites.** Since `demand + headroom = capacity` on every node, it rejects exactly when the removed capacity exceeds the total remaining headroom:

$$\sum_{i \in S}\text{capacity}_i \;>\; \sum_{\text{all } i}\text{headroom}_i$$

In a uniform cluster this is simply `k/N > (1 − ū)` — the removed *fraction* of nodes exceeds the cluster's *free fraction* (`ū` = mean utilization). So a small removal in a large, underutilized cluster always passes (`k/N → 0`), and the prune concentrates on **over-aggressive (big-k) removals and tight clusters**. Its leverage therefore *grows* with a more aggressive generator (e.g. the big-k merges of the repacking pivot) — the capacity check earns its keep precisely where a many-candidate generator needs it most.

#### 2. Cost — REPLACE worthwhileness · O(#instance types)

*If the pods don't fit as-is ($o_d(S) > 0$), the only option is one new node to hold the leftover — is that worth buying?* Karpenter adds at most one replacement (the *m→1* rule), and by conservation it must hold at least the overflow. So a worthwhile REPLACE needs an instance type that is both **big enough** and **cheaper** than what we removed. `REJECT the replace` if **no permitted instance type satisfies both conditions** (writing $I$ for a candidate type):
